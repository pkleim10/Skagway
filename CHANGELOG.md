...
- **Albums play as a playlist** — When an album is selected, finishing a video starts the next one in the album from the beginning (windowed, compact, and full screen). Missing files are skipped. **Play All** (⌘⇧P) starts the first video from the beginning. **Loop Album** (toolbar, View menu, or Settings) restarts the first video after the last one; off by default. The toolbar icon is two outline arrows chasing around a circle (accent when on).
