...
- **Fix: dragging the filters drawer's resize handle flickered.** Two causes: (1) the drag handler wrote directly to `vm.filtersDrawerHeight` on every `onChanged` delta — an `@Observable`, `UserDefaults`-backed property — so every pixel of movement meant a disk write plus broad view invalidation; decoupled into a local, non-persisted `filtersDrawerLiveHeight` that only commits to the real property once, on `onEnded`. (2) The real culprit: the handle is laid out *below* the drawer it resizes, so it moves as the drawer grows, and the `DragGesture`'s default *local* coordinate space made `translation` drift against the moving frame → an oscillating feedback loop. Switched to `coordinateSpace: .global` (measured against the fixed window frame) and rounded to whole points — the same fix the floating player's resize handle already uses.
- **New setting: "Filter drawer height" (Settings → Filters), `[Fit to content, Last used]`.** Fit to content sizes the drawer to exactly show all its filter cards (no scrollbar) and hides the resize handle — height isn't manually adjustable in this mode. Last used (default) reopens the drawer at whatever height you last dragged it to. Persisted across launches.
- **New: user-adjustable, persisted height for the filters drawer.** A drag handle appears at the bottom edge once the drawer is fully open; height persists across launches (`filtersDrawerHeight`, default 320pt, same as before). Minimum height is 110pt (~1.5in) — 1in (72pt) was tried first but cuts off a card's header plus its first row of content, which made the drawer useless at the floor; 110pt keeps at least that much visible. Two ceilings apply: the drawer can't be dragged taller than its own content (the point where the drawer's scrollbar disappears — growing past that would just add empty space), and it can't exceed what fits in the window (so the resize handle always stays on screen and reachable). The content ceiling is measured live, so it tracks as the number of filters or the column packing changes.
