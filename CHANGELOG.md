...
- **Fix: dragging the filters drawer's resize handle flickered.** The drag handler was writing directly to `vm.filtersDrawerHeight` on every `onChanged` delta — an `@Observable`, `UserDefaults`-backed property, so every pixel of movement meant a disk write plus broader view invalidation. Decoupled: drag now updates a local, non-persisted `filtersDrawerLiveHeight` for live visual feedback, and commits to `vm.filtersDrawerHeight` (one `UserDefaults` write) only once, in `onEnded`.
