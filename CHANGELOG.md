...
- **Fix: filters drawer handle's backdrop didn't match the Inspector hero handle's.** The capsule color was already unified, but the row around the filters drawer handle had no background of its own, so it showed the plain wall/window backdrop — a visibly lighter charcoal — instead of the consistent dark navy behind the Inspector's handle. Gave it an explicit background matching `CuratedWallInspector.inspectorBackground`.
- **An already-open compact player now tracks the hero resize handle in realtime**, instead of only snapping to the new size once the drag ends. The live drag value moved from local `@State` on the Inspector to a new, deliberately non-persisted `inspectorHeroLiveHeight` on `LibraryViewModel` — shared so `FloatingPlayerPanel` (a separate view) can read it too. Still never touches `UserDefaults` mid-drag; only the final value commits, on drag end.
- **Increased contrast on both resize handles (filters drawer + Inspector hero).** They were `Color.appDivider.opacity(0.5)` — `appDivider` is itself `Color.white.opacity(0.08)`, so the effective opacity was ~4%, nearly invisible. Changed to `Color.appTextSecondary.opacity(0.55)`.
