...
- **Cleaned up the menu bar's default macOS boilerplate** — SwiftUI apps get the entire standard menu bar for free, and only pieces this app explicitly touches get customized. Edit had Undo/Redo, Cut/Copy/Paste, and a Find/Spelling/Substitutions/Transformations/Speech submenu with nothing behind any of them; there was also an entire Format menu (Font/Text controls) irrelevant to a video library, and a dead "Print…" item. Removed all of it, and wired Select All/Deselect All (in place of Cut/Copy/Paste) to the same view-model calls the ⌘A/⌘⇧A shortcuts already use, so the menu items actually do something instead of sitting permanently disabled.
