...
- **Rebuilt the Edit menu's Cut/Copy/Paste/Select All/Deselect All/Delete/Remove from Library as one fully explicit group** instead of composing default system items with custom ones — the earlier composed approach produced a real, structural duplicate ("Select All" appearing twice) since the system default group's exact contents aren't something the app controls or fully knows ahead of time. Cut/Copy/Paste now forward to the standard responder-chain actions directly (same effective behavior as the system default, so they still work correctly in any focused text field), and Select All/Deselect All are disabled whenever a text field has focus so they don't show up as a redundant supplement inside that field's own native context menu.
- **Fixed ⌘A selecting all videos instead of a focused field's text when the field was empty.** The Wall grid's ⌘A handler only deferred to a focused text field when it already had text; an empty field (a custom metadata field, notes, rename, search) fell through to selecting all videos instead. Now always defers to a focused field regardless of content, so a keystroke typed while actively editing never redirects to the background.
