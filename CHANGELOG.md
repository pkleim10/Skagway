...
- **Cleaned up the menu bar's default macOS boilerplate** — SwiftUI apps get the entire standard menu bar for free, and only pieces this app explicitly touches get customized. Edit had Undo/Redo and a Find/Spelling/Substitutions/Transformations/Speech submenu with nothing behind any of them; there was also an entire Format menu (Font/Text controls) irrelevant to a video library, and a dead "Print…" item. Removed all of it, and added Select All/Deselect All to Edit, wired to the same view-model calls the ⌘A/⌘⇧A shortcuts already use. Kept Cut/Copy/Paste as-is — they're genuinely working text-editing commands whenever a field is focused (rename, notes, custom metadata, search), not dead weight.
- **Fixed the new Edit-menu "Select All" showing up as a duplicate inside a text field's own native Select All context menu** — being permanently enabled (unlike before, when nothing implemented it and it was harmlessly disabled) meant macOS offered it as a redundant supplement to any focused field's own right-click menu. Select All/Deselect All are now disabled whenever a text field has focus, matching the same check the ⌘A/⌘⇧A keyboard shortcuts already use.
