...
- **Fix: moving multiple selected videos could silently drop one from the selection.** `selectedVideoIds` is pruned to valid ids the instant GRDB's observation stream reflects a rename — which races ahead of the move code's own "was this video selected?" check that ran *after* the same rename's `await`. If the prune won the race, the check saw the old path already gone and skipped re-adding the new one, quietly unselecting that video (could hit any item in a multi-file move, not just one specific position). Fixed by capturing selection state before the DB write and applying it unconditionally afterward, in both the same-volume and cross-volume move paths. Pre-existing in `main`'s move code too, not introduced by the new queue.
